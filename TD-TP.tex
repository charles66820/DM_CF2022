...
void initPerm(int *P, unsigned int n);
    ...
    \begin{solutionorbox}
        On a en tout, 4 invariants : $I = I_1 \wedge I_2 \wedge I_3 \wedge I_4$ avec :\\
        $I_1 = (0 <= i <= n)$\\
        $I_2 = (unchangedTab\{Pre,Here\}(P,P,i,n))$\\
        $I_3 = (isPermutation(P,i))$\\
        $I_4 = (isMinPerm(P,i))$\\
        Tout d'abord on prouve :
        $$\breaktowidth{12.7cm}{\neg(i<n) \wedge I \Rightarrow \WLP(8,\psi)\\
        \equiv (i>=n) \wedge I \Rightarrow isPermutation(P,n) \wedge isMinPerm(P,n)}$$
