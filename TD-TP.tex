...
(unchangedTab{L,M}(t,s,0,n));
    ...
    \begin{solutionorbox}
        Pour le lemme \code{maxHasNoIncrease}:
        \begin{lstlisting}
\forall int *t, integer n; isMaxPerm(t,n) ==> \forall integer i; 0 <= i < n-1 ==> t[i] > t[i+1];
        \end{lstlisting}
        On vérifie que chaque élément du tableau t, soit supérieur à l'élément suivant dans le tableau.
        Comme dans \code{isMaxPerm}, on vérifie que la permutation est maximale, c'est-à-dire, que la
        permutation correspond à l'ordre inverse des indices dans le tableau alors ces derniers sont forcément
        ordonnés de manière décroissante donc le lemme \code{maxHasNoIncrease} est prouvé.
        Pour le lemme \code{increaseIsNoMax}:
        \begin{lstlisting}
\forall int *t, integer n; isPermutation(t,n) ==> (\exists integer i; 0 <= i < n-1 && t[i] < t[i+1]) ==> !isMaxPerm(t,n);
        \end{lstlisting}
        On vérifie qu'il existe un indice où l'élément est inférieur à l'élément qui suit dans le tableau. Comme dit précédemment,
        les éléments du tableau pour \code{isMaxPerm} sont forcément décroissants, donc l'implication est vraie.
        Donc le lemme \code{increaseIsNoMax} est également prouvé.
    \end{solutionorbox}
