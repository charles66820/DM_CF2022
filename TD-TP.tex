...
void swap(int *a, int *b)
{
    ...
    *a = (*a) ^ (*b);
    ...
    \begin{solutionorbox}
      \begin{align*}
        \WP(\code{swap},\psi) \equiv & ~\WP(6-8,\psi)                                                                                                                 \\
        ...
        \equiv                    & ~\top
      \end{align*}
      Donc on peut en déduire ce triplet de Hoare $= \{\top\}swap\{\psi\}$ avec $\psi \equiv \psi_1 \wedge \psi_2$\\
      et $\psi_1 = (*a == \old(*b))$ et $\psi_2 = (*b == \old(*a))$.
    \end{solutionorbox}
 ...
 unsigned int n)
{
    ...
    \begin{solutionorbox}
      ...
      \equiv i < j \land I \Rightarrow i+1 <= j \wedge i+1 >= i \wedge j-1 <= j $\\
      et on remarque que $i <= i+1 <= j <= j+1 \equiv \top$ donc l'implication est vraie.\\
      ...
      \begin{align*}
        ...
        \equiv               & ~\breaktowidth{8.2cm}{\WLP(10, \WLP(11, (\forall k; \old(i) <= k < i \Rightarrow t[k] == \old(t[\old(j)-k+\old(i)])) \wedge t[i] == \old(t[\old(j)-k+\old(i)]))}\\
        \equiv               & ~\breaktowidth{8.2cm}{\WLP(10, (\forall k; \old(i) <= k < i \Rightarrow t[k] == \old(t[\old(j)-k+\old(i)])) \wedge t[j] == \old(t[\old(j)-k+\old(i)])}\\
        \equiv               & ~\breaktowidth{8.2cm}{(\forall k; \old(i) <= k < i \Rightarrow t[k] == \old(t[\old(j)-k+\old(i)])) \wedge t[j] == \old(t[\old(j)-j+\old(i)])}\\
      \end{align*}
      Donc $i < j \land I \Rightarrow \WLP(10-14,I_3)\\
      \equiv i < j \land I \Rightarrow (\forall k; \old(i) <= k < i \Rightarrow t[k] == \old(t[\old(j)-k+\old(i)])) \wedge t[j] == \old(t[\old(j)-j+\old(i)])$\\
      or comme on évalue au début de la boucle, alors le \code{forall} n'est jamais exécuté et grâce à l'invariant $I_4$, on sait que dans la portion du tableau,
      pour chaque k entre la valeur initiale de $i$ et $i+1$, on a chaque $t[k] = \old(t[\old(j)-k+\old(i)])$ donc l'implication est vraie.\\
...
\forall integer i,j; 0 <= i < j < n ==> v[i] != v[j];
    ...
    \begin{solutionorbox}
        La ligne 2 du prédicat vérifie que chaque entier de $0$ à $n-1$ dans le tableau v, est bien associé à un entier
        entre $0$ et $n-1$. La ligne 3 du prédicat vérifie que chaque valeur associée dans le tableau v est unique, c'est-à-dire
...
void initPerm(int *P, unsigned int n) {
  ...
  */
  ...
  return;
        ...
        \begin{align*}
            \neg(i<n) \wedge I \Rightarrow \psi \equiv & ~\breaktowidth{5cm}{\neg(i<n) \wedge I_1 \wedge I_2 \wedge I_3 \wedge I_4 \Rightarrow \psi_1 \wedge \psi_2}\\
                                                \equiv & ~\breaktowidth{5cm}{(i>=n) \wedge (0 <= i <= n) \wedge unchangedTab\{Pre,Here\}(P,P,i,n) \wedge isPermutation(P,i) \wedge isMinPerm(P,i) \Rightarrow isPermutation(P,n) \wedge isMinPerm(P,n)}\\
                                                \equiv & ~\breaktowidth{5cm}{(i>=n) \wedge (0 <= i == n) \wedge unchangedTab\{Pre,Here\}(P,P,n,n) \wedge isPermutation(P,n) \wedge isMinPerm(P,n) \Rightarrow isPermutation(P,n) \wedge isMinPerm(P,n)}\\
                                                \equiv & ~\breaktowidth{5cm}{\top}
        \end{align*}
        ...
        On calcule :
        \begin{align*}
            \breaktowidth{9.5cm}{\WLP(15-16,I_1 \wedge I_2 \wedge I_3 \wedge I_4)}\\
            \equiv \breaktowidth{9cm}{\WLP(15, (0 <= i+1 <= n) \wedge unchangedTab\{Pre,Here\}(P,P,i+1,n) \wedge isPermutation(P,i+1) \wedge isMinPerm(P,i+1))}\\
            \equiv \breaktowidth{9cm}{((0 <= i+1 <= n)}\\
                   \breaktowidth{9cm}{\wedge unchangedTab\{Pre,Here\}(P,P,i+1,n)}\\
                   \breaktowidth{9cm}{\wedge isPermutation(P,i+1)}\\
                   \breaktowidth{9cm}{\wedge isMinPerm(P,i+1))[P[i] \leftarrow i]}\\
            \equiv \breaktowidth{9cm}{((0 <= i+1 <= n)}\\
                   \breaktowidth{9cm}{\wedge (\forall j; i+1 <= j < n \Rightarrow \old(P[j]) == P[j])}\\
                   \breaktowidth{9cm}{\wedge (\forall k; 0 <= k < i+1 \Rightarrow 0 <= P[k] < i+1 \wedge \forall k,j; 0 <= k < j < i+1 \Rightarrow P[k] != P[j])}\\
                   \breaktowidth{9cm}{\wedge (\forall k; 0 <= k < i+1 \Rightarrow k == P[k]))[P[i] \leftarrow i]}\\
            \equiv \breaktowidth{9cm}{((0 <= i+1 <= n)}\\
                   \breaktowidth{9cm}{\wedge ((\forall j; i <= j < n \Rightarrow P[j] == P[j]) \wedge P[i] == P[i])}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow 0 <= P[k] < i+1) \wedge 0 <= P[i] < i+1 \wedge (\forall k,j; 0 <= k < j < i \Rightarrow P[k] != P[j]) \wedge P[i-1] != P[i])}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow k == P[k]) \wedge i == P[i]))[P[i] \leftarrow i]}\\
            \equiv \breaktowidth{9cm}{(0 <= i+1 <= n)}\\
                   \breaktowidth{9cm}{\wedge ((\forall j; i <= j < n \Rightarrow P[j] == P[j]) \wedge i == i)}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow 0 <= P[k] < i+1) \wedge 0 <= i < i+1 \wedge (\forall k,j; 0 <= k < j < i \Rightarrow P[k] != P[j]) \wedge (i-1) != i)}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow k == P[k]) \wedge i == i)}\\
            \equiv \breaktowidth{9cm}{(0 <= i+1 <= n)}\\
                   \breaktowidth{9cm}{\wedge ((\forall j; i <= j < n \Rightarrow \top) \wedge \top)}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow 0 <= P[k] < i+1) \wedge \top \wedge (\forall k,j; 0 <= k < j < i \Rightarrow P[k] != P[j]) \wedge \top)}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow k == P[k]) \wedge \top)}\\
            \equiv \breaktowidth{9cm}{(0 <= i+1 <= n) \wedge \top}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow (0 <= P[k] < i+1 \wedge k == P[k])) \wedge (\forall k,j; 0 <= k < j < i \Rightarrow P[k] != P[j]))}\\
            \equiv \breaktowidth{9cm}{(0 <= i+1 <= n)}\\
                   \breaktowidth{9cm}{\wedge ((\forall k; 0 <= k < i \Rightarrow (0 <= k < i+1 \wedge k == P[k])) \wedge (\forall k,j; 0 <= k < j < i \Rightarrow P[k] != P[j]))}\\
            \equiv \breaktowidth{9cm}{(0 <= i+1 <= n) \wedge ((\forall k; 0 <= k < i \Rightarrow (\top \wedge k == P[k])) \wedge (\forall k,j; 0 <= k < j < i \Rightarrow P[k] != P[j]))}\\
        \end{align*}
...
bool isMaxPerm(int *P, unsigned int n);
    ...
    \begin{solutionorbox}
        Nous avons tout démontré pour la fonction \code{isMaxPerm} ci-dessus.
    \end{solutionorbox}
